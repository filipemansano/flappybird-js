...
let scoreColor = ["purple", "white"]; // cores do player no light e dark mode

let minBarHeight = 10; // altura minina de uma coluna
let maxBarHeight = 150; // altura maxima de uma coluna
let barWidth = 40; // largura da coluna
...
let heightBarrelHead = 15;
barrel.src = "images/barrel.png";
player.src = "images/player1.jpg";
...
function drawArea() {
    ...
    for (let i = 0; i < bars.length; i++) {
        ...
        canvasArea.drawImage(barrel, 40, (height-bar.top.h), barWidth, bar.top.h, bar.top.x, bar.top.y, bar.top.w, bar.top.h);
        canvasArea.drawImage(barrel, 0, 0, barWidth, bar.bottom.h, bar.bottom.x, bar.bottom.y, bar.bottom.w, bar.bottom.h);
    }


    canvasArea.font = scoreFont;
    canvasArea.fillStyle = currentScoreColor;
    canvasArea.fillText("Score: " + parseInt(score+=0.1), width - 170, 30);
...
function drawBox() {
    ...
    canvasBox.drawImage(player, box.x, box.y, box.w, box.h);
}
...
function colisionCheck() {
    ...
    let BoxXPixelEnd = (box.x + box.w);
    let BoxYPixelEnd = (box.y + box.h);

    // Se a posição X do player for maior ou igual a posição X da primeria barra
    if ( BoxXPixelEnd >= firstBar.top.x && box.x <= (firstBar.top.x + barWidth)) {

        /**
         * Se o 1º pixel do eixo Y do player for menor ou igual a altura do cano do topo -> colisão
         * Se o ultimo pixel pixel do eixo Y do player for maior ou igual ao 1º pixel da barra do rodapé -> colisão
         */
        let colisaoTopo     = box.y <= firstBar.top.h;
        let colisaoRodape   = BoxYPixelEnd >= firstBar.bottom.y;

        if (colisaoTopo || colisaoRodape) {

            // Verificando se a batida foi acima da cabeça do cano do topo
            if(colisaoTopo){

                if(BoxYPixelEnd < (firstBar.top.h - heightBarrelHead) && BoxXPixelEnd < firstBar.top.x + 5){
                    return;
                }

            }

            // Verificando se a batida foi abaixo da cabeça do cano do rodape
            else if(box.y > (firstBar.bottom.y + heightBarrelHead) && BoxXPixelEnd < firstBar.top.x + 5){
                    return;
            }
...
function start() {
    ...
    barsComplete = 0;
    currentColorScheme = 0;
    backgroundPanel.className = "content";
...
function switchPlayer(n){
    let div = document.getElementById('player'+n);
    
    Array.prototype.forEach.call(document.getElementsByClassName("players"), function(el) {
        el.className = "players";
    });

    player.src = "images/player" +n+ ".jpg";
    div.className = "players playerActive";
}
/////////////// Players //////////////////////
...
function playBackGroundMusic(){
    ...
    if(soundController && bgSoundSource !== undefined){
